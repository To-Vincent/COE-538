...
DISPATCHER      CMPA  #START                    ; If it’s the START state ------------------+
                ...
                BRA   DISP_EXIT                 ; and exit                                  I
                                                ;                                           S
NOT_START       CMPA  #FWD                      ; Else if it’s FORWARD state                P
                BNE   NOT_FORWARD               ;                                           A
                JSR   FWD_ST                    ; call FORWARD routine                      T
                JMP   DISP_EXIT                 ; exit                                      C
                                                ;                                           H
NOT_FORWARD     CMPA  #REV                      ; Else if it’s REVERSE state                E
                BNE   NOT_REVERSE               ;                                           R
                JSR   REV_ST                    ; call REVERSE routine                      |
                JMP   DISP_EXIT                 ; exit                                      |
                                                ;                                           |
NOT_REVERSE     CMPA  #ALL_STP                  ; Else if it’s ALL_STOP state               |
                BNE   NOT_ALL_STOP              ;                                           |
                JSR   ALL_STP_ST                ; call ALL_STOP routine                     |
                JMP   DISP_EXIT                 ; exit                                      |
                                                ;                                           |
NOT_ALL_STOP    CMPA  #FWD_TRN                  ; Else if it’s the FORWARD_TURN state       |
                BNE   NOT_FWD_TRN               ;                                           |
                JSR   FWD_TRN_ST                ; call FORWARD_TURN routine                 |
                JMP   DISP_EXIT                 ; exit                                      |
                                                ;                                           |
...
FWD_ST          BRSET PORTAD0,$04,NO_FWD_BUMP   ; If FWD_BUMP then
                ...
                MOVB  #REV,CRNT_STATE           ; set state to REVERSE
                JMP   FWD_EXIT                  ; return

NO_FWD_BUMP     BRSET PORTAD0,$08,NO_REAR_BUMP  ; If REAR_BUMP, then
                JSR   INIT_ALL_STP              ; initialize ALL_STOP routuine
...
NO_REAR_BUMP    LDAA  TOF_COUNTER               ; If Tc>Tfwd then
                CMPA  T_FWD                     
                BNE   NO_FWD_TRN                
                JSR   INIT_FWD_TRN              ; initialize FORWARD_TURN state
                MOVB  #FWD_TRN,CRNT_STATE       ; change state to FORWARD_TURN
                JMP   FWD_EXIT                  ; return
...
initLCD         BSET  DDRB,%11111111            ; configure pins PB7,...,PB0 for output
                ...
                JSR   cmd2LCD                   
                LDAA  #$0C                      ; display on, cursor and blinking off
                JSR   cmd2LCD                   
...
cmd2LCD:        BCLR  LCD_CNTR,LCD_RS           ; select the LCD Instruction Register
                JSR   dataMov                   ; send data to Instruction Register
      	        RTS
...
;********************************************************************************************
putcLCD         BSET  LCD_CNTR,LCD_RS           ; select LCD Data register
                JSR   dataMov                   ; send data to Data register
                RTS
                
;********************************************************************************************
dataMov         BSET  LCD_CNTR,LCD_E            ; pull LCD E-sigal high
                STAA  LCD_DAT                   ; send upper 4 bits to LCD
                BCLR  LCD_CNTR,LCD_E            ; pull LCD E-signal low to complete the write oper.
                LSLA                            ; match the lower 4 bits with the LCD data pins
                LSLA                            
                LSLA                            
                LSLA                            
                BSET  LCD_CNTR,LCD_E            ; pull LCD E signal high
                STAA  LCD_DAT                   ; send lower 4 bits to LCD
                BCLR  LCD_CNTR,LCD_E            ; pull LCD E-signal low to complete the write oper.
                LDY   #1                        ; delay to finish oper.
                JSR   del_50us                   
                RTS
...
int2BCD         XGDX                            ; Save binary number into X
                ...
                BEQ   CON_EXIT                  ; if true, exit

                XGDX                            ; else, get the binary number back to D as dividend
                LDX   #10                       ; Setup 10 (in decimal) as the divisor
                ...
                LDX   #10                       ; setup for another division by 10
                IDIV
                STAB  TENS
                CPX   #0
                BEQ   CON_EXIT
      ...
      CON_EXIT: RTS                             ; finished conversion
...
BCD2ASC         LDAA  #$0                       ; Initialize the blanking flag
      ...
      ISBLANK1: LDAA  #$20                      ; It’s blank
                STAA  TEN_THOUS                 ; store a space
                BRA   C_THOU                    ; check the ’thousands’ digit

    NOT_BLANK1: LDAA  TEN_THOUS                 ; Get the ’ten_thousands’ digit
                ORAA  #$30                      ; Convert to ASCII
                STAA  TEN_THOUS
                LDAA  #$1                       ; show ’non-blank’ digit exists
                STAA  NO_BLANK

        C_THOU: LDAA  THOUSANDS                 ; Check the thousands digit for blank
    ...
    NOT_BLANK2: LDAA  THOUSANDS                 ; same process to ’ten_thousands’ case
                ORAA  #$30
                STAA  THOUSANDS
                LDAA  #$1
                STAA  NO_BLANK
    ...
    NOT_BLANK3: LDAA  HUNDREDS                  ; same process to ’ten_thousands’ case
                ORAA  #$30
                STAA  HUNDREDS
                LDAA  #$1
                STAA  NO_BLANK
      ...
      ISBLANK4: LDAA  #$20                      ; Tens digit blank
                STAA  TENS                      ; store a space
                BRA   C_UNITS                   ; check the units digit

    NOT_BLANK4: LDAA  TENS                      ; same process to ’ten_thousands’ case
                ORAA  #$30
                STAA  TENS

       C_UNITS: LDAA  UNITS                     ; convert to ASCII
                ORAA  #$30
                STAA  UNITS
...
UPDT_DISPL      MOVB  #$90,ATDCTL5              ; R-just., uns., sing. conv., mult., ch=0, start
                BRCLR ATDSTAT0,$80,*            ; Wait until conver. seq. is complete
                ...
                JSR   cmd2LCD                                 
                LDAA  TEN_THOUS                 ; output the TEN_THOUS ASCII char.
                JSR   putcLCD                   
                LDAA  THOUSANDS                 ; output the THOUSANDS ASCII char.
                JSR   putcLCD                   
                LDAA  #$2E                      ; output the HUNDREDS ASCII char.
                JSR   putcLCD                   
                LDAA  HUNDREDS                  ; output the HUNDREDS ASCII char.
                JSR   putcLCD                                  
;--------------------------------------------------------------------------------------------
